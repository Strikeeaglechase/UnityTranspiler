...
import { compressRpcPackets } from "vtol-live-viewer-shared/dist/compression/vtcompression.js";
...
import { EntityEvent, EntityInitData, EventType } from "./events.js";

const inputPath = "C:/Users/strik/Desktop/Programs/CSharp/UnityGERunner/bin/Debug/net6.0/recording.json";
const mapPath = "C:/Users/strik/Desktop/Programs/CSharp/UnityGERunner/src/Application/Resources/Map";
const outputPath = "C:/Users/strik/Desktop/Programs/CSharp/UnityGERunner/UnityTranspiler/output.vtgr";
...
class Converter {
	...
	public convert(inputFile: string) {
		...
		const initEvents = data.flatMap(frame => frame.events.filter(e => e.type == EventType.EntityInit && e.path.startsWith("Vehicles/")));
		initEvents.forEach((init: EntityInitData) => {
		...
		console.log(`Generated ${playerInfos.length} player infos`);
		...
		const vtgr = this.buildVtgrFile(lobbyId);
		return vtgr;
	}
	...
	private handleEvent(event: EntityEvent) {
		switch (event.type) {
			case EventType.EntityInit: {
				...
				console.log(`Instantiating entity ${event.entityId} of type ${ctor.name} at path ${event.path}`);
			...
			case EventType.EntityDelete: {
				...
				console.log(`Destroyed entity ${event.entityId}`);
			}
			case EventType.Flare:
			case EventType.Chaff: {
				const entity = this.entities[event.entityId];
				if (!entity) {
					console.log(`Entity ${event.entityId} not found for chaff event`);
					return;
				}
			...
			case EventType.RemoveDebugShape:
				...
				if (!entityToRemove) {
					console.log(`Entity ${event.id} not found for debug shape removal`);
					return;
				}
			...
			default:
				console.log(`Unhandled event type: ${EventType[event.type]} (${event.type})`);
		}
	}
	...
	private handleFrame(frame: RecordedFrame) {
		...
		frame.motion.forEach(motion => {
			...
			if (!entity) {
				console.log(`Entity ${motion.entityId} not found for aircraft update`);
				return;
			}
	...
	private buildVtgrFile(lobbyId: string) {
		...
		console.log(`Chunked RPCs into ${chunkedRpcs.length} windows`);

		const compressedData = chunkedRpcs.map(chunk => compressRpcPackets(chunk, true));
		// fs.writeFileSync("../rpcs.json", JSON.stringify(this.rpcs, null, 2));
		// const decompData = decompressRpcPackets(Buffer.from(compressedData[0]));
		// console.log(decompData[0]);
	...
	private onRpc<T = unknown>(className: string, method: string, id: string | number, args: any[], classObj: T) {
		// console.log(`(${id}) ${className}.${method}(${args.join(", ")})`);
		this.rpcs.push({
			className: className,
			method: method,
			args: args,
...
function copyEmittedFile(filename: string) {
	const filePath = path.join(path.dirname(inputPath), filename);
	const outputFilePath = path.join(path.dirname(outputPath), filename);
...
function convert() {
	...
	console.log(`Converting recording from ${inputPath} to ${outputPath}`);
	const vtgr = converter.convert(inputPath);
	vtgr.pipe(writeStream);

	copyEmittedFile("graphs.json");
	copyEmittedFile("state.json");
}
